...
void flux_r_soa2(struct domain *theDomain, double dt)
{
    ...
    int j0 = NgRa > 0 ? NgRa-1 : 0;
    int j1 = NgRb > 0 ? Nr-NgRb+1 : Nr;
    ...
    for(k=NgZa; k<Nz-NgZb; k++)
    {
        ...
        for(j=j0; j<j1; j++)
        {
            ...
            if(j > j0)
            {
                ...
                const double *f_dphi_L = theDomain->fr_dphi_L[jk];
                const double *f_offset_L = theDomain->fr_offset_L[jk];
                ...
                for(i=0; i<Np[jk]; i++)
                {
                    ...
                    double xL[3] = {rL, piph[i]+f_offset_L[i]-0.5*dphiL[iL], z};
                    double xm[3] = {rm, piph[i]-f_dphi_L[i], zm};
            ...
            if(j < j1-1)
            {
                ...
                double rR = get_centroid(rpp, rp, 1);
                ...
                const double *f_dphi_R = theDomain->fr_dphi_R[jk];
                const double *f_offset_R = theDomain->fr_offset_R[jk];
                ...
                for(i=0; i<Np[jk]; i++)
                {
                    int iR = f_iR[i];
                    double x[3] = {r, piph[i]-0.5*dphi[i], z};
                    double xR[3] = {rR, piph[i]+f_offset_R[i]-0.5*dphiR[iR], z};
                    double xm[3] = {rp, piph[i]-f_dphi_R[i], zm};
    ...
    for(k=NgZa; k<Nz-NgZb; k++)
    {
        ...
        for(j=NgRa; j<Nr-NgRb; j++)
        {
            ...
            if(k > 0)
            {
                ...
                const double *f_dphi_L = theDomain->fz_dphi_L[jk];
                const double *f_offset_L = theDomain->fz_offset_L[jk];
                ...
                for(i=0; i<Np[jk]; i++)
                {
                    int iL = f_iL[i];
                    double x[3] = {r, piph[i]-0.5*dphi[i], z};
                    double xL[3] = {r, piph[i]+f_offset_L[i]-0.5*dphiL[iL], zL};
                    double xm[3] = {rm, piph[i]-f_dphi_L[i], zm};
            ...
            if(k < Nz-1)
            {
                ...
                const double *f_dphi_R = theDomain->fz_dphi_R[jk];
                const double *f_offset_R = theDomain->fz_offset_R[jk];
                ...
                for(i=0; i<Np[jk]; i++)
                {
                    int iR = f_iR[i];
                    double x[3] = {r, piph[i]-0.5*dphi[i], z};
                    double xR[3] = {r, piph[i]+f_offset_R[i]-0.5*dphiR[iR], zR};
                    double xm[3] = {rm, piph[i]-f_dphi_R[i], zp};
                    double xp[3] = {rp, piph[i], zp};
